...
# Bar plot for proportion of papers affected in top 6 journals with more than 100 gene name errors articles from 2014 to 2020
df_propotion_of_artcle_affected<-unique(jjf[,c(1,4)])
df_propotion_of_artcle_affected
dff<-df_propotion_of_artcle_affected[1:6,]
barplot(dff[,2],names.arg = dff[,1],ylab="proportion of papers affected in top 6 journals %",ylim = c(0,50),las=3)
grid(nx=0,ny=1,col = "gray")

dev.off()

par(mfrow=c(3,1))
# line diagrams on the number of gene name error articles published by  journal per yea
...
# line diagram for proportion of papers affected in top 6 journals with more than 100 gene name errors articles from 2014 to 2020
df_propotion_of_artcle_affected<-unique(jjf[,c(1,4)])
dff<-df_propotion_of_artcle_affected[1:6,]
barplot(dff[,2],names.arg = dff[,1],ylab="proportion of papers affected in top 6 journals %",ylim = c(0,50),las=3)
grid(nx=0,ny=0,col = "gray")
```
